...
public class IfNotFriendsTag extends MbsTagSupport {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -6493069067323102354L;
	private User member;
	...
	public int doStartTag() throws JspException {
		...
		try{			
			...
			if(!member.getProfile().getFriends().contains(currentUser)){
				//we are not friends! So show this content
				return EVAL_BODY_INCLUDE;
			}else{
				//we are friends, so don't show this content
				return SKIP_BODY;
			}
		}catch(Exception e){
			//we aren't logged in
